...
enum Report {
    static func dryRun(_ router: Router) {
        ...
        if let web = router.webURL {
            line("Host", web.host)
        } else {
            line("Host", "— not an http(s) URL")
        }
        ...
        line("Installed", router.appURL.map { $0.path } ?? "NOT FOUND")
    ...
    static func describe(_ reason: PassthroughReason) -> String {
        switch reason {
